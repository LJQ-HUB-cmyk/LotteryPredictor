...
def simulate_lottery(data, window_size, m_range, n_range):
    results = []
    ...
    for m in m_range:
        for n in n_range:
            winnings = 0
            for start_day in range(total_days - window_size):
                ...
                selected_numbers = random_walk_lottery(window_num_by_day, m, n, window_frequency)
                actual_numbers = set(num_by_day[-(start_day + window_size+1)])
                match_count = len(set(selected_numbers) & actual_numbers)   
                if match_count == 3:
                    winnings += 30
                elif match_count == 4:
                    winnings += 300
                    ...
                    winnings += 12000000  # Assuming a large jackpot for illustration
                    print(f"Jackpot! {selected_numbers} on day {total_days - (start_day + window_size+1)}")
                else:
                    winnings -= 10
            results.append((m, n, winnings))
    return results

last_appearance_range = range(0, 30)  # Example values for m
top_freq_range = range(6, 20) 
print("Simulating lottery...")
frequency, num_by_day, stripped_data = freq_reversed_freq(data)
results = simulate_lottery(data, 400, last_appearance_range, top_freq_range) 
best_strategy = max(results, key=lambda x: x[2])
print(f"Best strategy: Considering the numbers that have not appeared in the last {best_strategy[0]} days and the top {best_strategy[1]} most frequently appearing numbers")
total_days = len(num_by_day)
start_day = 0
...
window_num_by_day = num_by_day[-window_size-start_day:]
window_frequency, _, _ = freq_reversed_freq("\n".join(window_data))
selected_numbers = random_walk_lottery(window_num_by_day, best_strategy[0], best_strategy[1], window_frequency)
